...
#define VIDEO_FOLDER @"video" //视频录制存放文件夹
#define kScreenWidth [UIScreen mainScreen].bounds.size.width
#define kScreenHeight [UIScreen mainScreen].bounds.size.height

@interface LSSVideoModel()<AVCaptureVideoDataOutputSampleBufferDelegate, AVCaptureAudioDataOutputSampleBufferDelegate, AVAssetWriteManagerDelegate>
...
- (void)clearFile
{
    [self removeItemAtPath:[self videoFolder] error:nil];
}
...
- (NSString *)videoFolder
{
    ...
    if (![self isExistsAtPath:cacheDir]) {
        [self createDirectoryAtPath:cacheDir error:nil];
    }
    return cacheDir;
}
...
#pragma mark -初始化writer， 用writer 把数据写入文件
- (void)setUpWriter
{
    self.videoUrl = [[NSURL alloc] initFileURLWithPath:[self createVideoFilePath]];
    self.writeManager = [[LSSVAssetWriteManager alloc] initWithURL:self.videoUrl viewType:_viewType WithPerPixel:_bitsPerPixel WithRecordMaxTime:_recordMaxTime];
