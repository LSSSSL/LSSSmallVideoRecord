...
@interface LSSVAssetWriteManager()
...
@implementation LSSVAssetWriteManager
...
- (void)setUpWriter
{
    ...
    //每像素比特
    CGFloat perbit;
    if (_bitsPerPixel == FMVideoViewBiteHeight) {
        perbit = 8.0;
    }else if (_bitsPerPixel == FMVideoViewBiteMedium)
    {
        perbit = 5.0;
    }else
    {
    ...
    NSInteger bitsPerSecond = numPixels * perbit;
    // 码率和帧率设置
...
- (void)destroyWrite
{
    ...
    [self.timer invalidate];
    self.timer = nil;
    self.bitsPerPixel = FMVideoViewBiteHeight;
    self.recordMaxTime = 15;
}
